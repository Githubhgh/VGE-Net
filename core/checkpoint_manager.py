...
class CheckpointManager(object):
    ...
    def load_all(self, dir = None):
        ...
        if dir == None:
            dir = self._model.output_path
            
        iteration_number = 0
        iteration_numbers = []
        for prefix, saver in self._savers.items():
            output_path = '%s/checkpoints/%s' % (dir, prefix)
    ...
    def save_all(self, iteration_number, i=0):
        """Save all prefixes."""
        prefixes_to_use = []
        ...
        for prefix, saver in self._savers.items():
            ...
            saver.save(self._tensorflow_session, output_path + '/model_'+str(i),
