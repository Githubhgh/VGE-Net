...
class BaseModel(object):
    ...
    def _build_optimizers(self):
        ...
        self.lr_kl_pre = self._learning_schedule[0]['learning_rate']
        self.global_step = tf.Variable(0, trainable=False, name="global_step")
        self.lr = tf.placeholder(tf.float32, shape=())

        for i, spec in enumerate(self._learning_schedule):
            optimize_ops = []
            update_ops = []
            ...
            for loss_term_key, prefixes in loss_terms.items():
                ...
                optimizer = optimizer_class(
                    learning_rate=self.lr_kl_pre*self.learning_rate_multiplier,
                )
    ...
    def train(self, num_epochs=None, num_steps=None, now_leave_one_test=None, return_end=False):
        ...
        if num_steps is None:
            ...
            epoch_step = num_entries / self._batch_size
        ...
        try:
            ...
            for current_step in range(initial_step, num_steps):
                ...
                fetches['metrics'] = self.metrics['train']['kl_loss']
                fetches['out'] = self.output_tensors['train']['kl_loss']
                fetches['angle'] = self.metrics['train']['gaze_ang']
                ...
                current_epoches = current_step // epoch_step

                # Print progress
                to_print = 'The experiments for %07d： %07d / %07d>' % (now_leave_one_test, current_step, num_steps,)
                to_print += ', '.join(['%s = %g' % (k, v)
                                       for k, v in zip(loss_term_keys, outcome['loss_terms'])])
                to_print += ', '.join([' kl loss is  ' + str(outcome['out'])])
                to_print += ', '.join([' train angle error is  ' + str(outcome['angle'])])
        ...
        print('Best result is ' + str(self._tester.best_result))
        logger.info('Best result is ' + str(self._tester.best_result))
