...
class LiveTester(object):
    ...
    def do_full_test(self, sleep_between_batches=0.2, return_final=True):
        ...
        # Reset data sources
        for data_source_name, data_source in self.data.items():###self.data传入test_data
            data_source.reset()
        ...
        for i in range(num_batches):
            ...
            try:
                ...
                time_test = 1e3*(time.time() - start_time)
        ...
        import sys
        if 'gaze_ang' in outputs.keys():
            nni.report_intermediate_result(outputs['gaze_ang'])
        if self.best_result > outputs['gaze_ang']:
            self.best_result = outputs['gaze_ang']
            logger.info("Best Result now is " + str(self.best_result))
        ...
        if return_final==True:
            return outputs['gaze_ang']

        return False

    def do_final_full_test(self, current_step, return_final=True):
        ...
        nni.report_final_result(self.best_result)
        logger.info('Best result is ' + str(self.best_result))
        self.copy_model_weights()
        self._is_testing = True
        self._testing_at_step = current_step
        self.do_full_test(sleep_between_batches=0.5)

        if return_final==True:
            angle_current = self.do_full_test(sleep_between_batches=0.5, return_final=return_final)
